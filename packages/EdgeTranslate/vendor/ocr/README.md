...
-   `packages/EdgeTranslate/scripts/sync-ocr-assets.js`
    -   构建时从 npm 包生成审核友好的 worker，并复制官方 core 文件到
        `build/<browser>/ocr/` 或 `dev/<browser>/ocr/`。
...
corePath: chrome.runtime.getURL("ocr/core/tesseract-core-lstm.js");
```
...
Chrome Web Store 审核会扫描扩展包中的 JavaScript。Tesseract 官方发布的
`worker.min.js` 和 `*.wasm.js` 会包含 base64 编码的 wasm 检测或 wasm 内容，容易被
自动审核判成混淆代码；构建脚本改为生成未压缩 worker，并使用外部 `.wasm` 文件。
生成后的 worker 还会移除 webpack/regenerator 的动态执行 fallback，避免扩展包里出现
不必要的 `Function` 构造调用。
