...
`pdfjs-dist` 提供库运行时、worker、cmaps、字体、wasm 解码器和 viewer 图片。
它不提供完整 generic viewer app 文件，例如 `web/viewer.html`、
`web/viewer.mjs`、`web/viewer.css` 和 viewer locale 目录。这些文件继续保留在
...
6. 检查生成的 `packages/EdgeTranslate/build/chrome/pdf` 输出：
    ...
    - `pdf/web/viewer.mjs` 包含扩展来源 URL 校验 patch。
