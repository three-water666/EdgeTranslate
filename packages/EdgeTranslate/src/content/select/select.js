...
function initSelectTranslate() {
    ...
    const BLOCK_TEXT_MAX_LENGTH = 500;
    const BLOCK_TEXT_MIN_LENGTH = 8;
    const BLOCK_TEXT_IDEAL_LENGTH = 150;
    const DIRECT_TEXT_BLOCK_TAG_REGEXP = /^(P|LI|BLOCKQUOTE|PRE|DD|DT|FIGCAPTION|CAPTION|H[1-6])$/;
    const ImageData =
    ...
    let longPressPreventClickTarget = null;
    let longPressPreventClickUntil = 0;
    ...
    window.addEventListener("DOMContentLoaded", () => {
        ...
        document.addEventListener("click", longPressClickHandler, true);
    ...
    function longPressClickHandler(event) {
        if (!longPressPreventClickTarget || Date.now() > longPressPreventClickUntil) {
    ...
    function getPreferredBlockContainer(textNode, x, y) {
        const directTextBlockContainer = getDirectTextBlockContainer(textNode);
        if (directTextBlockContainer) return directTextBlockContainer;
        ...
        let bestScore = getBlockContainerScore(fallbackContainer, x, y, 0);
    ...
    function getBlockContainerScore(element, x, y, depth = 0) {
        ...
        // Collect text to analyze quality
        ...
        // 1. Tag-aware Ideal Length Proximity
        let ideal = BLOCK_TEXT_IDEAL_LENGTH;
        if (/^(H[1-6])$/.test(element.tagName)) ideal = 40;
        else if (/^(LI|TD|TH)$/.test(element.tagName)) ideal = 60;

        let score = 200 - Math.abs(ideal - textLength) / 2;

        // 2. Natural Language Bonus (Punctuation)
        // Paragraphs with punctuation are more likely to be meaningful content
        const punctuationCount = (fullText.match(/[.!?。！？；;，,]/g) || []).length;
        score += Math.min(punctuationCount, 10) * 15;

        // 3. Tag importance
        if (/^(P|LI|BLOCKQUOTE|PRE)$/.test(element.tagName)) {
            score += 150;
        } else if (/^(H[1-6]|TD|TH|CAPTION)$/.test(element.tagName)) {
            score += 100;
        }

        // 4. Complexity Penalty: Favor pure text blocks over layout containers
        if (element.childElementCount > 0) {
            score -= element.childElementCount * 20;
        }

        // 5. Depth Penalty: Prefer the inner-most element when scores are similar
        score -= depth * 10;
    ...
    function isReasonableBlockContainer(element) {
        ...
        // Relax minimum length for specific short-text containers
